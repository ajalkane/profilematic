...
Configuration::writeRules(const QList<Rule> &rules) {
    ...
    for (int i = 0; i < rules.size(); ++i) {
        ...
        s.setValue("networkMode", int(r.condition().getNetworkMode()));
...
Configuration::readRules(QList<Rule> &rules, int *rules_version_return) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        RuleCondition::NetworkMode networkMode
                = (RuleCondition::NetworkMode) s.value("action", 0).toInt();
        r.condition().setNetworkMode(networkMode);
